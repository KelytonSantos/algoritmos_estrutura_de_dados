...
void pos_ordem(no *raiz)
{
    ...
    while (raiz != NULL || !pilha_vazia(novapilha))
    {
        ...
        else
        {
            ...
            if (!topo(novapilha, &topo_atual))
                break;

            if (topo_atual->dir != NULL && ultimo_visitado != topo_atual->dir)
            {
                raiz = topo_atual->dir;
            }
            else
            {
                printf("%d ", topo_atual->dado);
                desempilha(novapilha, &topo_atual);
                ultimo_visitado = topo_atual;
            }
        }
    }
}
